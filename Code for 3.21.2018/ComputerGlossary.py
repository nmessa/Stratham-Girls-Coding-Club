...
# Idea 1 - accessing a dictionary entry randomly
# Flash Cards App
...
def click1():
    questions = list(my_glossary.keys()) # put all the keys into a list so that their indexes are numerical
    question = random.choice(questions) # choose a random question
    entry.delete(0, END)  # clear input text box (there are no columns in Entry boxes hence 0 rather than 0.0)
