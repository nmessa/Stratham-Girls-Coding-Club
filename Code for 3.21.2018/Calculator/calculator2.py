...
def click(key):
    ...
    # now for the function keys:
        ...
        if key == functions_list[0]:
            display.insert(END, calc_functions2.factorial(n))

        elif key == functions_list[1]:
            display.insert(END, calc_functions2.to_roman(n))

        elif key == functions_list[2]:
            display.insert(END, calc_functions2.to_binary(n))

        else:
            display.insert(END, calc_functions2.from_binary(n))

    # add other key-pressed values to end of current display:
    else:
...
# use display widget for an editable display
