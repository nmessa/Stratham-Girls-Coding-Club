# Add a square and square root button
...
def click(key):
    ...
    # now for the function buttons:
        ...
        display.insert(END, myFunctions.factorial(n))

    elif key == functions_list[1]:
        n = display.get()  # collect display value
        display.delete(0, END)  # clear display
        display.insert(END, myFunctions.to_roman(n))

    elif key == functions_list[2]:
        n = display.get()  # collect display value
        display.delete(0, END)  # clear display
        display.insert(END, myFunctions.square(n))
        
    elif key == functions_list[3]:
        n = display.get()  # collect display value
        display.delete(0, END)  # clear display
        display.insert(END, myFunctions.square_root(n))

    elif key == functions_list[4]:
        n = display.get()  # collect display value
        display.delete(0, END)  # clear display
        display.insert(END, myFunctions.to_binary(n))

    elif key == functions_list[5]:
        n = display.get()  # collect display value
        display.delete(0, END)  # clear display
        display.insert(END, myFunctions.from_binary(n))
